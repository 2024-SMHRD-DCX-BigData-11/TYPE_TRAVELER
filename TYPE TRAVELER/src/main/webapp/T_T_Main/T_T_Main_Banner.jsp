...
<body>
    <div class="header">
        ...
        <ul class="nav-menu">
            ...
            <li><a href="javascript:void(0);" onclick="checkLoginAndRedirect();">일정관리</a></li>
            <li>
                <a>커뮤니티</a>
                <ul class="submenu">
                    <li><a onclick="message()">T_T 모집</a></li>
                    <li><a href="${pageContext.request.contextPath}/TravelLogCon">여행기록</a></li>
                </ul>
            </li>
            <c:choose>
    ...
    <script>
    var isLoggedIn = "<%= session.getAttribute("memberInfo") != null ? "true" : "false" %>";
    var contextPath = "<%= request.getContextPath() %>";
        ...
        function openLoginModal(redirectURL = null) {
            if (redirectURL) {
                ...
                document.getElementById('redirectURL').value = window.location.href.replace('loginError=true', '');
            }
    ...
    <script>
        ...
        function checkEmail() {
            ...
            console.log("Checking email:", inputEmail);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data) {
                        $('#resultCheck').text('사용할 수 없는 이메일').css('color', 'red');
                    } else {
                        $('#resultCheck').text('사용할 수 있는 이메일').css('color', 'green');
                    }
                },
                error: function() {
                    alert('error');
        ...
        function submitJoinForm() {
            ...
            xhr.open("POST", "${pageContext.request.contextPath}/JoinCon", true);
            ...
            xhr.send(`user_email=${email}&user_pw=${password}&user_name=${name}&user_mbti=${mbti}`);
        }
      ...
      <div id="myPageModal" style="display: none;">
          <div id="myPageContent">
              ...
              <form id="profileForm" action="${pageContext.request.contextPath}/UpdateCon" method="post" onsubmit="return validatePassword()">
                  ...
                  <button type="button" class="update-button" onclick="updateProfile(event)">프로필 수정</button>
              </form>
          </div>
      </div>
         ...
         console.log(json_data)
            fetch(updateUrl,{
               ...
               headers: {
                ...
                // body에 현재 비번, 새로운 비번, 사용자 이름, 사용자 이메일 받는 것
                body: 'currentPassword=' + currentPassword + '&newPassword=' + newPassword + 
                	  '&user_name=' + user_name + '&user_mbti=' + user_mbti
            }).then(function(response){ 
                return response.json(); 
                
            }).then(function(json) {
                if(json.res){
                   alert("현재 비밀번호를 잘못 입력했습니다. 다시 입력해주세요.");
                } else {
                    alert("변경되었습니다.");
                    closeMyPageModal();
                }
          ...
          /*  const result = await response.json();
           if (result.success) {
              alert(123)
               ...
               // 실패 시 에러 메시지 표시
               alert(result.error); // 서버에서 반환한 오류 메시지를 알림으로 표시
               document.getElementById('passwordErrorMessage').style.display = 'block';
           }
       try {
           const response = await fetch(updateUrl, {
